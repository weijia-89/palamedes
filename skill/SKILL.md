...
**Procedural fix / setup guide (single-file HTML).** If the user asks for a step-by-step guide to fix, build, install, or configure something (auto repair, RAG PC, Signal, self-hosted stack, appliance), load `references/procedural-guide-site.md`. Copy `templates/procedural-guide/template.html`, run P1→P4 on load-bearing specs, fill mandatory section IDs (`#start-here`, `#shopping` with `#wf-*` workflows, `#bench-setup`, `#walkthrough` Parts A–K with symmetric reinstall, `#restore-*`, `#verify`). Worked example: `~/Downloads/2015-forester-ac-clutch-fix-guide.html`. Deliver self-contained HTML (inline CSS) for offline/phone use. Reassembly and tool organization are first-class — not optional appendices.
...
| `references/agentic-research.md` | parallel branches / harness / worktree / RAG / debate / **Pattern 7** + **Pattern 8** + **Pattern 9** |
...
| `references/literature-corpus-fanout.md` | **Pattern 9** — PDF/text corpus; one sub-agent per paper; critical ingest → index → synthesis |
| `references/authoritative-review-literacy.md` | systematic review vs meta-analysis vs meta-review; AUTH-1 checklist; routing weight for review papers |
| `references/prompts/literature-paper-ingest.md` | Pattern 9 sub-agent prompt (paper ingest, index writer, synthesizer) |
| `references/rag-eval-literacy.md` | §TRIGGER in file: Ragas / RAG eval / faithfulness / context recall / ≥2 automated metrics |
| `references/synthesizer-agent.md` | multi-lane merge; manifest ingest; context-rot prevention; Pattern 8 + Pattern 9 wave graphs |
...
| `references/procedural-guide-site.md` | user asks for a fix / install / setup walkthrough as offline single-file HTML (remove + reinstall + bench layout + workflow shopping) |
